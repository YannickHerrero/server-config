...
When asking the user to run Ansible, `./bin/check`, or `./bin/apply`, offer in the same message to handle the work through a maintenance window. State whether a normal or sensitive window is required. If a suitable window is already active, use `server-config-window converge` yourself instead of asking the user to run those commands. Use `server-config-window converge --scope NAME` only when the commit is confined to one allowlisted non-sensitive area. Use full convergence for cross-cutting or host-access changes.

A maintenance window is standing application approval only until its displayed deadline. Never open or extend one on the user's behalf. Confirm its scope with `server-config-window status`. Scoped convergence always blocks access, firewall, SSH, Tailscale, sudo, and user-account tasks. A normal window blocks those tasks plus Citadel and maintenance-window tasks. The user must open a sensitive window to authorize host-access categories during full convergence. Citadel and maintenance-window changes always use the ordinary interactive apply path. A window does not authorize pushing or releasing commits.
